...
public class HotpotSoupBaseRecipe extends AbstractHotpotCommonInputRecipe {
    ...
    public boolean matches(HotpotRecipeInput container, @NotNull Level level) {
        return ingredient.test(container.itemStack())
                && sourceSoupTypeKey.equals(container.soup().soupTypeHolder().getKey());
    }
    ...
    public static class Serializer implements RecipeSerializer<HotpotSoupBaseRecipe> {
        public static final MapCodec<HotpotSoupBaseRecipe> CODEC =
                LazyMapCodec.of(() -> RecordCodecBuilder.mapCodec(recipe -> recipe.group(
                                HotpotComponentSoupType.KEY_CODEC
                                        .fieldOf("source_soup")
                                        .forGetter(HotpotSoupBaseRecipe::getSourceSoupTypeKey),
                                HotpotComponentSoupType.KEY_CODEC
                                        .fieldOf("result_soup")
                                        .forGetter(HotpotSoupBaseRecipe::getResultSoupTypeKey),
                                Codec.FLOAT
                                        .fieldOf("result_water_level")
                                        .forGetter(HotpotSoupBaseRecipe::getResultWaterLevel),
                                Ingredient.CODEC_NONEMPTY
                                        .fieldOf("ingredient")
                                        .forGetter(HotpotSoupBaseRecipe::getIngredient),
                                ItemStack.OPTIONAL_CODEC
                                        .optionalFieldOf("remaining_item", ItemStack.EMPTY)
                                        .forGetter(HotpotSoupBaseRecipe::getRemainingItem),
                                SoundEvent.CODEC.fieldOf("sound_event").forGetter(HotpotSoupBaseRecipe::getSoundEvent))
                        .apply(recipe, HotpotSoupBaseRecipe::new)));

        public static final StreamCodec<RegistryFriendlyByteBuf, HotpotSoupBaseRecipe> STREAM_CODEC =
                NeoForgeStreamCodecs.lazy(() -> StreamCodec.composite(
                        HotpotComponentSoupType.KEY_STREAM_CODEC,
                        HotpotSoupBaseRecipe::getSourceSoupTypeKey,
                        HotpotComponentSoupType.KEY_STREAM_CODEC,
                        HotpotSoupBaseRecipe::getResultSoupTypeKey,
                        ByteBufCodecs.FLOAT,
                        HotpotSoupBaseRecipe::getResultWaterLevel,
                        Ingredient.CONTENTS_STREAM_CODEC,
                        HotpotSoupBaseRecipe::getIngredient,
                        ItemStack.OPTIONAL_STREAM_CODEC,
                        HotpotSoupBaseRecipe::getRemainingItem,
                        SoundEvent.STREAM_CODEC,
                        HotpotSoupBaseRecipe::getSoundEvent,
