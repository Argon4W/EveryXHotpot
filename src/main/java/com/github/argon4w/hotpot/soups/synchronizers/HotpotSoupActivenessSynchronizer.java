...
public class HotpotSoupActivenessSynchronizer implements IHotpotSoupSynchronizer {
    ...
    public void collect(HotpotBlockEntity hotpotBlockEntity, BlockPosWithLevel pos) {
        ...
        if (soup instanceof IHotpotSoupWithActiveness) {
            collectedActiveness += ((IHotpotSoupWithActiveness) soup).getActiveness(hotpotBlockEntity, pos);
        }
    }
    ...
    public void integrate(int size, HotpotBlockEntity hotpotBlockEntity, BlockPosWithLevel pos) {
        ...
        if (hotpotBlockEntity.getSoup() instanceof IHotpotSoupWithActiveness) {
            ((IHotpotSoupWithActiveness) hotpotBlockEntity.getSoup()).setActiveness(hotpotBlockEntity, pos, averageActiveness);
        }
    }
}
