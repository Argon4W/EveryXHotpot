...
public record HotpotComponentSoup(
        LinkedHashMap<ResourceLocation, IndexHolder<IHotpotSoupComponent>> components,
        Holder<HotpotComponentSoupType> soupTypeHolder) {
    @SuppressWarnings("unchecked")
    public <T extends IHotpotSoupComponent> List<Pair<ResourceLocation, T>> getComponentPairsByTypes(
            List<Supplier<? extends IHotpotSoupComponentTypeSerializer<? extends T>>> componentTypeSerializerHolders) {
        return soupTypeHolder.value().getComponentKeysByTypes(componentTypeSerializerHolders).stream()
    ...
    public <T extends IHotpotSoupComponent> List<Pair<ResourceLocation, T>> getComponentPairsByType(
            Supplier<? extends IHotpotSoupComponentTypeSerializer<? extends T>> componentTypeSerializerHolder) {
        return getComponentPairsByTypes(List.of(componentTypeSerializerHolder));
    }

    public <T extends IHotpotSoupComponent> List<T> getComponentsByTypes(
            List<Supplier<? extends IHotpotSoupComponentTypeSerializer<? extends T>>> componentTypeSerializerHolders) {
    ...
    public <T extends IHotpotSoupComponent> List<T> getComponentsByType(
            Supplier<? extends IHotpotSoupComponentTypeSerializer<? extends T>> componentTypeSerializerHolder) {
        return getComponentPairsByType(componentTypeSerializerHolder).stream()
                .map(Pair::getSecond)
                .toList();
    }
    ...
    public Optional<Pair<ResourceLocation, IHotpotSoupComponent>> getComponentPair(ResourceLocation resourceLocation) {
        return components.get(resourceLocation) == null
                ? Optional.empty()
                : Optional.of(Pair.of(
                        resourceLocation, components.get(resourceLocation).value()));
    }
    ...
    public <T> IHotpotResult<T> getResultFromComponents(
            IHotpotResult<T> defaultResult,
            BiFunction<IHotpotSoupComponent, IHotpotResult<T>, IHotpotResult<T>> function) {
        return components.values().stream()
                .map(IndexHolder::value)
                .collect(
                        () -> new AtomicReference<>(defaultResult),
                        (reference, component) -> reference.set(
                                reference.get().isBlocked()
                                        ? reference.get()
                                        : function.apply(component, reference.get())),
                        (reference1, reference2) -> reference1.set(reference2.get()))
                .get();
    }
    ...
    public LinkedHashMap<ResourceLocation, IndexHolder<IHotpotSoupComponent>> getPartialComponents() {
        return components.sequencedEntrySet().stream()
                .filter(EntryStreams.filterEntryValue(holder -> holder.value().shouldSendToClient()))
    ...
    public IHotpotResult<Holder<IHotpotContentSerializer<?>>> getPlayerInteractionResult(
            ...
            LevelBlockPos pos) {
        return getResultFromComponents((component, result) -> component.getPlayerInteractionResult(
                position, player, hand, itemStack, this, pos, result, hotpotBlockEntity));
    }
    ...
    public IHotpotResult<Holder<IHotpotContentSerializer<?>>> getContentSerializerResultFromItemStack(
        ...
        return getResultFromComponents((component, result) ->
                component.getContentSerializerResultFromItemStack(itemStack, hotpotBlockEntity, this, pos, result));
    }

    public IHotpotResult<IHotpotContent> getContentResultByTableware(
            IHotpotContent content, HotpotBlockEntity hotpotBlockEntity, LevelBlockPos pos) {
    ...
    public IHotpotResult<IHotpotContent> getContentResultByHand(
            IHotpotResult<IHotpotContent> content, HotpotBlockEntity hotpotBlockEntity, LevelBlockPos pos) {
        return getResultFromComponents(
                content, (component, result) -> component.getContentResultByHand(hotpotBlockEntity, this, pos, result));
    }

    public double getContentTickSpeed(HotpotBlockEntity hotpotBlockEntity, LevelBlockPos pos) {
        return getResultFromComponents(
                        IHotpotResult.success(0.0),
                        (component, result) -> component.getContentTickSpeed(hotpotBlockEntity, this, pos, result))
                .orElse(0.0);
    }

    public List<IHotpotSoupComponentSynchronizer> getSoupComponentSynchronizers(
            HotpotBlockEntity hotpotBlockEntity, LevelBlockPos pos) {
        return components.values().stream()
                .map(IndexHolder::value)
                .map(component -> component.getSoupComponentSynchronizer(hotpotBlockEntity, this, pos))
                .filter(Optional::isPresent)
                .map(Optional::get)
                .toList();
    }
    ...
    public boolean isHotpotLit(HotpotBlockEntity hotpotBlockEntity, LevelBlockPos pos) {
        return getResultFromComponents(
                        ...
                        (component, result) -> component.getHotpotLit(hotpotBlockEntity, this, pos, result))
                .orElse(true);
    }

    public double getWaterLevel() {
        return getResultFromComponents(IHotpotResult.pass(), IHotpotSoupComponent::getWaterLevel)
                .orElse(0.0);
    }

    public double getOverflowWaterLevel() {
        return getResultFromComponents(IHotpotResult.pass(), IHotpotSoupComponent::getOverflowWaterLevel)
                .orElse(0.0);
    }
