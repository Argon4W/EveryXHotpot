...
public class AdditionalSectionGeometryBlockEntityRendererDispatcher implements AddSectionGeometryEvent.AdditionalSectionRenderer {
    ...
    public AdditionalSectionGeometryBlockEntityRendererDispatcher(BlockPos regionOrigin) {
    ...
    public RendererBakedModelsCache createCache(IBlockEntitySectionGeometryRenderer<?> renderer) {
        return renderer instanceof ICustomRendererBakedModelsCacheProvider provider
                ? provider.createCache()
                : new DefaultRendererBakedModelsCache();
    }
    ...
    public void renderAt(BlockPos pos, AddSectionGeometryEvent.SectionRenderingContext context) {
        ...
        if (!(Minecraft.getInstance().getBlockEntityRenderDispatcher().getRenderer(blockEntity)
                instanceof IBlockEntitySectionGeometryRenderer<?> renderer)) {
            return;
        }
        ...
        try {
            renderer.renderSectionGeometry(
                    ...
                    pos,
                    regionOrigin,
                    new LightAwareSectionGeometryRenderContext(context, getOrCreateCache(renderer), pos));
