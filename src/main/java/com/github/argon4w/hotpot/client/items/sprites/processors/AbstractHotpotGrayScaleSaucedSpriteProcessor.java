...
public abstract class AbstractHotpotGrayScaleSaucedSpriteProcessor implements IHotpotSpriteProcessor {
    ...
    private double getAverageGrayScale(NativeImage image, FrameSize frameSize, int frame) {
        return getPointStream(frameSize)
                .mapToInt(point -> image.getPixelRGBA(point.x, point.y + frame * frameSize.height()))
                .filter(color -> FastColor.ARGB32.alpha(color) != 0)
                .mapToDouble(this::getGrayScale)
                .average()
                .orElse(0d);
    }

    public double getGrayScale(int color) {
        return (FastColor.ABGR32.red(color) * 0.299
                        + FastColor.ABGR32.green(color) * 0.587
                        + FastColor.ABGR32.blue(color) * 0.144)
                / 255.0;
    }
