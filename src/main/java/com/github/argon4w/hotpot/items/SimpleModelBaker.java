...
public record SimpleModelBaker(Map<ResourceLocation, BakedModel> bakedModels, Map<ResourceLocation, UnbakedModel> models, UnbakedModel missingModel, Function<Material, TextureAtlasSprite> spriteGetter) implements ModelBaker {
    @NotNull
    @Override
    ...
    public static ResourceLocation getCheesedLocation(ResourceLocation location) {
        return location instanceof ModelResourceLocation modelLocation ?
                new ModelResourceLocation(new ResourceLocation(location.getNamespace(), location.getPath().concat("_cheesed")), modelLocation.getVariant())
                : new ResourceLocation(location.getNamespace(), location.getPath().concat("_cheesed"));
    }
}
