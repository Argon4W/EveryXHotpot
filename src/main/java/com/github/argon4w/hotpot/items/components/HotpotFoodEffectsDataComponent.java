...
public record HotpotFoodEffectsDataComponent(HotpotMobEffectMap effects) {
    public static final HotpotFoodEffectsDataComponent EMPTY =
            new HotpotFoodEffectsDataComponent(new HotpotMobEffectMap());

    public static final Codec<HotpotFoodEffectsDataComponent> CODEC =
            Codec.lazyInitialized(() -> RecordCodecBuilder.create(data -> data.group(HotpotMobEffectMap.CODEC
                            .fieldOf("effects")
                            .forGetter(HotpotFoodEffectsDataComponent::effects))
                    .apply(data, HotpotFoodEffectsDataComponent::new)));

    public static final StreamCodec<RegistryFriendlyByteBuf, HotpotFoodEffectsDataComponent> STREAM_CODEC =
            NeoForgeStreamCodecs.lazy(() -> StreamCodec.composite(
                    HotpotMobEffectMap.STREAM_CODEC,
                    HotpotFoodEffectsDataComponent::effects,
    ...
    public static HotpotFoodEffectsDataComponent getDataComponent(ItemStack itemStack) {
        return itemStack.getOrDefault(
                HotpotModEntry.HOTPOT_FOOD_EFFECTS_DATA_COMPONENT, HotpotFoodEffectsDataComponent.EMPTY);
    }
    ...
    public static boolean hasEffects(ItemStack itemStack) {
        return hasDataComponent(itemStack)
                && !getDataComponent(itemStack).effects.isEmpty();
    }
