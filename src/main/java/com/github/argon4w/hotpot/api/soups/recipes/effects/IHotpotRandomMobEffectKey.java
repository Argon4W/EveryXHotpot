...
public interface IHotpotRandomMobEffectKey {
    ...
    StreamCodec<ByteBuf, Wrapper> STREAM_CODEC =
            ByteBufCodecs.either(Range.STREAM_CODEC, Value.STREAM_CODEC).map(Wrapper::new, Wrapper::either);
    ...
    record Range(int from, int to) implements IHotpotRandomMobEffectKey {
        ...
        public static final StreamCodec<ByteBuf, Range> STREAM_CODEC =
                StreamCodec.composite(ByteBufCodecs.INT, Range::from, ByteBufCodecs.INT, Range::to, Range::new);
    ...
    record Value(int value) implements IHotpotRandomMobEffectKey {
        public static final Codec<Value> CODEC =
                Codec.INT.fieldOf("value").xmap(Value::new, Value::value).codec();
