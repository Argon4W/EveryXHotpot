...
public class HotpotPlayerContent implements IHotpotContent {
    ...
    private void reloadModelPartWithSkin(int index) {
        slim = DefaultPlayerSkin.get(profile).model().id().equals("slim");
        modelSkin = DefaultPlayerSkin.getDefaultTexture();

        Minecraft.getInstance().getSkinManager().getOrLoad(profile).thenAcceptAsync(playerSkin -> {
            slim = playerSkin.model().id().equals("slim");
            modelSkin = playerSkin.texture();
