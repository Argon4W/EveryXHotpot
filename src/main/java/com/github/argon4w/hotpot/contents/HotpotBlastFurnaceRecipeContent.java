...
public class HotpotBlastFurnaceRecipeContent extends AbstractHotpotCookingRecipeContent {
    ...
    public Optional<BlastingRecipe> getRecipe(ItemStack itemStack, BlockPosWithLevel pos) {
        return  pos.level().getRecipeManager().getRecipeFor(HotpotContents.BlAST_FURNACE_COOKING_RECIPE, new SimpleContainer(itemStack), pos.level());
    }
    ...
    public static boolean hasBlastingRecipe(ItemStack itemStack, BlockPosWithLevel pos) {
        return  pos.level().getRecipeManager().getRecipeFor(HotpotContents.BlAST_FURNACE_COOKING_RECIPE, new SimpleContainer(itemStack), pos.level()).isPresent();
    }
}
