...
public class HotpotContents {
    ...
    public static final StreamCodec<RegistryFriendlyByteBuf, IHotpotContentFactory<?>> STREAM_CODEC = NeoForgeStreamCodecs.lazy(() -> ByteBufCodecs.registry(CONTENT_REGISTRY_KEY));

    public static final Codec<IHotpotContentFactory<?>> FACTORY_CODEC = Codec.lazyInitialized(() -> getContentRegistry().byNameCodec());
    public static final MapCodec<IHotpotContent> CODEC = LazyMapCodec.of(() -> FACTORY_CODEC.dispatchMap(IHotpotContent::getFactory, IHotpotContentFactory::buildFromCodec));
    public static final Codec<List<HotpotContentWithSlot>> SLOT_CODEC = Codec.lazyInitialized(() -> Codec.INT.dispatch("slot", HotpotContentWithSlot::slot, i -> CODEC.xmap(c -> new HotpotContentWithSlot(i, c), HotpotContentWithSlot::content)).listOf());
