...
public class HotpotElegantPlacementRackBlockEntity
        extends AbstractHotpotPlacementBlockEntity<HotpotElegantPlacementRackBlockEntity.Data, HotpotElegantPlacementRackBlockEntity.PartialData>
    ...
    public static final List<Integer> PROVIDED_POSITIONS_GROUND_SOUTH = IntStream.range(0, 16)
            .boxed()
            .filter(Predicate.not(List.of(5, 6)::contains))
            .toList();
    public static final List<Integer> PROVIDED_POSITIONS_GROUND_NORTH = IntStream.range(0, 16)
            .boxed()
            .filter(Predicate.not(List.of(9, 10)::contains))
            .toList();
    public static final List<Integer> PROVIDED_POSITIONS_GROUND_WEST = IntStream.range(0, 16)
            .boxed()
            .filter(Predicate.not(List.of(5, 9)::contains))
            .toList();
    public static final List<Integer> PROVIDED_POSITIONS_GROUND_EAST = IntStream.range(0, 16)
            .boxed()
            .filter(Predicate.not(List.of(6, 10)::contains))
            .toList();

    public static final List<List<Integer>> PROVIDED_POSITIONS_RACK_BY_INDEX = List.of(
            PROVIDED_POSITIONS_RACK_SOUTH,
            PROVIDED_POSITIONS_RACK_WEST,
            PROVIDED_POSITIONS_RACK_NORTH,
            PROVIDED_POSITIONS_RACK_EAST);
    public static final List<List<Integer>> PROVIDED_POSITIONS_GROUND_BY_INDEX = List.of(
            PROVIDED_POSITIONS_GROUND_SOUTH,
            PROVIDED_POSITIONS_GROUND_WEST,
            PROVIDED_POSITIONS_GROUND_NORTH,
            PROVIDED_POSITIONS_GROUND_EAST);
    private static final Object2IntMap<BlockState> STATE_TO_INDEX = new Object2IntOpenHashMap<>();

    private boolean contentChanged;

    public HotpotElegantPlacementRackBlockEntity(BlockPos blockPos, BlockState blockState) {
        super(HotpotModEntry.HOTPOT_ELEGANT_PLACEMENT_RACK_BLOCK_ENTITY.get(), blockPos, blockState);
    ...
    public List<Integer> getProvidedPositions(int layer, LevelBlockPos pos) {
        return switch (layer) {
            ...
            case 1 -> PROVIDED_POSITIONS_RACK_BY_INDEX.get(getStateIndex(pos));
            case 2 -> PROVIDED_POSITIONS_RACK_BY_INDEX.get(getStateIndex(pos));
            default -> List.of();
        };
    }
    ...
    public void place(IHotpotPlacement placement, int position, int layer, LevelBlockPos pos) {
        getPlacements(layer).add(placement);
        markDataChanged();
    }
    ...
    public PartialData getPartialData(HolderLookup.Provider registryAccess) {
        return new PartialData(
                contentChanged ? Optional.of(data.placements0) : Optional.empty(),
    ...
    public int getPlacementIndexInPosAndLayer(int position, int layer) {
        ...
        return IntStream.range(0, placements.size())
    ...
    public int getStateIndex(LevelBlockPos pos) {
        return STATE_TO_INDEX.computeIntIfAbsent(pos.getBlockState(), this::getDirectionIndex);
    }

    public int getDirectionIndex(BlockState blockState) {
        return blockState.getValue(HotpotElegantPlacementRackBlock.FACING).get2DDataValue();
    }
    ...
    public static void tick(
            ...
            BlockPos pos,
            BlockState state,
            HotpotElegantPlacementRackBlockEntity blockEntity) {
    ...
    public record Data(
            LinkedList<IHotpotPlacement> placements0,
    ...
    public record PartialData(
            Optional<LinkedList<IHotpotPlacement>> placements0,
        ...
        public Data update(Data data) {
            return new Data(
                    placements0.orElse(data.placements0),
